...
class OpPreparation {
 public:
  static void PrepareTensorWithFormat(phi::DenseTensor& tensor, aclFormat format);
  ...
  static std::string DebugString(const phi::DenseTensor& tensor) {
    ...
    ss << ", origin_format: " << npu_properties.origin_format 
       << ", origin_dims: [" << npu_properties.origin_dims << "]"
       << ", storage_format: " << npu_properties.storage_format
